...
numbers will give different types produced by the inductive family \mb{Vec}. For example, (\mb{Vec\ A\ zero}) is
...
\mb{\exists x\in A.\ B(x)} to be true, a proof x of \mb{A}, and a proof of the predicate
...
a partial automaton. It was implemented using the \mb{Maybe} type as follow:
\begin{lstlisting}[mathescape=true,xleftmargin=.3\textwidth]
