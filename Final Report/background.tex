...
\par Unlike in Haskell, we must provide a type signature explicitly for every
...
\par What should we return in case \mb{[\ ]}? In Haskell, we can
...
possible workaround here is to return \mb{nothing} of the \mb{Maybe} type
...
type that depends on \mb{n}. With different natural numbers, we get
different types from the inductive family \mb{Vec}. For example, \mb{Vec\ A\ zero} is
...
\par We only need to pattern match on the case (\mb{x
:: xs}) because the type \mb{Vec\ A\ (suc\ n)} ensures that the argument will never
...
defined as a function that transforms any arbitrary proof of \mb{A} to
...
\paragraph{Implication} We say that \mb{A} implies \mb{B} if and only if we can
transform every proof of \mb{A} into a proof of \mb{B}. In Type
...
\mb{A}. We can consider it as a conjunction of the two implications.
\begin{lstlisting}[mathescape=true,xleftmargin=.3\textwidth]
...
implication. In order for \mb{\forall x\in A.\ B(x)} to be true, we will have
to transform every proof \mb{(a)} of \mb{A} into a proof of the predicate
...
\mb{\exists x\in A.\ B(x)} to be true, we need to provide a proof
\mb{(a)} of \mb{A} and a proof \mb{(p)} of the predicate
\mb{B[x:=a]}. In Type Theory, it is represented by the generalised
...
\par \(All\hyphen lt\) defines the condition where a given
...
\paragraph{} While in 2013, Firsov and Uustalu also published another related
