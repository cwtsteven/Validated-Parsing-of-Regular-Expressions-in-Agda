...
open import Util hiding (mytail)
...
∈-lem₁ x .x as (here refl) a∉as         = refl
...
(a ∈? (.a ∷ as)) {dec} | yes refl = yes (here refl)
...
∈-lem₂ : {A B : Set}{n : ℕ}(decA : DecEq A)(decB : DecEq B)(f : A → B) → (a : A)(as : Vec A n)
         → (a ∈ as) → (f a ∈ map f as)
∈-lem₂ decA decB f a ._ (here refl)               = here refl
...
∈-lem₃ dec x (.x ∷ as) bs (here refl) = inj₁ (here refl)
...
∈-lem₄ : {A : Set}{n m : ℕ}(dec : DecEq A)(a : A)(as : Vec A n)(bs : Vec A m)
         → a ∈ as → a ∈ as ++ bs
∈-lem₄ dec x (.x ∷ as) bs (here refl) = here refl
...
∈-lem₅ dec a (.a ∷ as) bs a∈bs | yes refl = here refl
...
any-lem₂ []       P ( a , Pa , ())
any-lem₂ (a ∷ as) P (.a , Pa , (here refl))       = inj₁ Pa
...
all-lem₁ []       P tt                    = λ a ()
all-lem₁ (a ∷ as) P (Pa , allas) .a  (here refl) = Pa
...
module Vec-Rep {A : Set}{n : ℕ}(dec : DecEq A)(It : Vec A (suc n))(∀a∈It : ∀ a → a ∈ It)(unique : Unique It) where
  ...
  It-lem₃ (a ∷ as) bs ._ refl refl .a (here refl) = inj₁ (here refl)
  ...
  It-lem₂ as (x ∷ bs) prf It≡asbs .x a∉as | inj₂ (here refl) = inj₁ refl
  ...
  It-lem₅ (a ∷ x ∷ as) (a∉xas , uni-xas) .a a∈as refl a∈mytail = ⊥-elim (a∉xas a∈mytail)

  It-lem₇ : {n : ℕ}(as : Vec A (suc n))
            → ∀ a → a ∈ as → a ≡ head as ⊎ a ∈ tail as
  It-lem₇ (a ∷ []) .a (here refl) = inj₁ refl
  It-lem₇ (a ∷ []) b (there ())
  It-lem₇ (a ∷ x ∷ as) .a (here refl) = inj₁ refl
  ...
  It-lem₄ a a≢head | inj₂ a∈mytail = a∈mytail
