...
parseToNFA Ø = record { Q = Ø-State ; δ = λ _ _ _ → ⊥ ; q₀ = init ; F = ø ; F? = λ _ → no (λ ()) }
parseToNFA ε = record { Q = Q' ; δ = δ' ; q₀ = init ; F = F' ; F? = F?' } 
 where
  Q' : Set
  Q' = ε-State
  δ' : Q' → Σᵉ → Subset Q'
  ...
  F?' init  = yes tt
  F?' error = no (λ ())
parseToNFA (σ a) = record { Q = Q' ; δ = δ' ; q₀ = init ; F = F' ; F? = F?' }
 where
  Q' : Set
  Q' = σ-State
  δ' : Q' → Σᵉ → Subset Q'
  ...
  F?' accept = yes tt
  F?' error  = no (λ ())
parseToNFA (e₁ ∣ e₂) = record { Q = Q' ; δ = δ' ; q₀ = init ; F = F' ; F? = F?' }
 where
  open NFA (parseToNFA e₁) renaming (Q to Q₁ ; δ to δ₁ ; q₀ to q₀₁ ; F to F₁ ; F? to F₁?)
  open NFA (parseToNFA e₂) renaming (Q to Q₂ ; δ to δ₂ ; q₀ to q₀₂ ; F to F₂ ; F? to F₂?)
  Q' : Set
  Q' = Q₁ ⊍ Q₂
  δ' : Q' → Σᵉ → Subset Q'
  ...
  F?' (⊍inj₁ q) = F₁? q
  F?' (⊍inj₂ q) = F₂? q
parseToNFA (e₁ ∙ e₂) = record { Q = Q' ; δ = δ' ; q₀ = ⍟inj₁ q₀₁ ; F = F' ; F? = F?' }
  where
   open NFA (parseToNFA e₁) renaming (Q to Q₁ ; δ to δ₁ ; q₀ to q₀₁ ; F to F₁ ; F? to F₁?)
   open NFA (parseToNFA e₂) renaming (Q to Q₂ ; δ to δ₂ ; q₀ to q₀₂ ; F to F₂ ; F? to F₂?)
   Q' : Set
   ...
   F?' mid       = no (λ ())
   F?' (⍟inj₂ q) = F₂? q
parseToNFA (e *)     = record { Q = Q' ; δ = δ' ; q₀ = init ; F = F' ; F? = F?' }
 where
...
remove-ε-setp : NFA → NFA
remove-ε-setp nfa = record { Q = Q ; δ = δ' ; q₀ = q₀ ; F = F ; F? = F? }
 where
  open NFA nfa
  δ' : Q → Σᵉ → Subset Q
  δ' q E     = λ p → ∀ a q' → q' ∈ δ q E × p ∈ δ q' a
  δ' q (α a) = δ q (α a)
...
parseToDFA = powerset-construction ∘ remove-ε-setp ∘ parseToNFA
